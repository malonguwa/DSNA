...
public class SVNLogExtractor {
	...
	public static void getCode(String path, String localPath, String projectName) {
		...
		try {
			Process process = rt.exec("cmd /c start " + localPath + "/"
					+ projectName + ".bat");
	...
	public static void analysisLog(String projectName, String localPath) {
		...
		try {
			pw = new PrintWriter(new OutputStreamWriter(new FileOutputStream(
					localPath + "\\" + projectName + ".DSN", false)));
	...
	private static void generateBATFile1(String path, String localPath,
		...
		try {
			PrintWriter pw = new PrintWriter(new OutputStreamWriter(
					new FileOutputStream(localPath + "\\" + projectName
							+ ".bat", false)));
			String line = "set SVNROOT=" + path;
			pw.println(line);
	...
	private static void generateBATFile2(String path, String localPath,
		...
		try {
			...
			for (String filePath : fileList) {
				...
				if (!Common.isTarget(fileName))
					continue;
				line = "svn log " + fileName + " > \"" + localPath + "\\"
						+ projectName + "_log\\" + fileName.replace("/", "_") + ".log\"";
				pw.println(line);
			}
			line = "exit";
			pw.println(line);
	...
	public static List<String> getSubFile(String fileName, String path,
			String projectName) {
	...
	public static List<EditInfo> parseLog(String logPath) {
		...
		FileReader fr;
		Pattern authorPattern = Pattern.compile("r[0-9]* \\| (.+?) \\|");
		Pattern timePattern = Pattern.compile(".+? \\| .+? \\| (.+?) \\|");
		// Pattern fileNamePattern = Pattern.compile("author: (.+?);");
		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (matcher.find()) {
					...
					dateTime = df.parse(matcher.group(1).substring(0, "0000-00-00 00:00:00".length()));
				}
	...
	public static void main(String[] args) {
		...
		try {
			FileReader fr = new FileReader("D:\\report\\job2012_2\\projects\\svn_list.txt");
			BufferedReader br = new BufferedReader(fr);
			String line = "";
			while ((line = br.readLine()) != null) {
				String segs[] = line.trim().split("\t");
				//SVNLogExtractor.getCode(segs[1], localPath, segs[0]);
				SVNLogExtractor.getLog(segs[1], localPath, segs[0]);
				SVNLogExtractor.analysisLog(segs[0], localPath);
			}
			br.close();
		} catch (FileNotFoundException e) {
			// TODO Auto-generated catch block
